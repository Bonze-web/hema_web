...
function fix(str, length, place, left) {
  ...
  var times = Math.floor(len / place.length);
  var arr = [];
...
export function formatDate(date, frm = 'yyyy-MM-dd HH:mm:ss') {
  ...
  var n = date.getFullYear() < 0 ? '-' : '';
  var yearStr = n + fix(year.toString(), 4, '0', true);
  var y = parseInt(yearStr.substr(yearStr.length - 2));
  ...
  var g = year % 100 === 0 ? (year / 100 + 1) : Math.ceil(year / 100);
...
*/
export function parseDate(dateStr, frm = 'yyyy-MM-dd HH:mm:ss') {
  if (!dateStr) {
    return null;
  }
  if (!frm) {
...
export function formatSimpleDate(v) {
  const now = new Date();
  const yd = new Date(now.getTime() - 24 * 60 * 60 * 1000);
  const tw = new Date(now.getTime() + 24 * 60 * 60 * 1000);
 ...
 * @param v 日期+时间
 */
export function simpleDateTime(v) {
  if (!v) {
    return '';
  } else {
    const d = new Date(v);
    return formatSimpleDate(v) + formatDate(d, " hh:mm");
  }
}

export function simpleTime(v) {
  if (!v) {
    return '';
  } else {
    const d = new Date(v);
